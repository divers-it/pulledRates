...
The following function returns the three pulled rates. Arguments are the birth and death rates (functions), the sampling fraction (real number) and a time series for integration. The default value is a constant birth-death model with complete sampling.
...
pulled <- function(birth,death,rho,t) {
  ...
  PDR <- birth(t[-length(t)]) - death(t[-length(t)]) - diff(birth(t))/(dt*birth(t[-length(t)]))   # Pulled diversification rate
...
A function to generate four figures for a given scenario. 
...
# True rates
#Ymin <- 0.9*min(spe(t), ext(t),div(t)) - 0.03
#Ymax <- 1.1*max(spe(t), ext(t),div(t)) + 0.06

Ymin <- -0.1
Ymax <- 0.25

plot(spe(-t)~t,type="l",col=2,ylim=c(Ymin,Ymax),ylab = "", cex.lab = 0.75, cex.axis=0.75, xlab = bquote("Age (" * tau * ")"),
  xaxt = "n"
)
axis(1, at = pretty(range(t)), labels = rev(pretty(range(-t))), cex.axis=0.75)
lines(ext(-t)~t,col=1)
lines(div(-t)~t,col="black")
legend(
  "topright",
...
#Ymin <- 0.9*min(div(t), PDR) - 0.03
#Ymax <- 1.1*max(div(t), PDR) + 0.06
plot(div(-t)~t,type="l",col="black",ylim=c(Ymin,Ymax),ylab = "",cex.lab = 0.75, cex.axis=0.75, xlab = bquote("Age (" * tau * ")"),
  xaxt = "n"
)
axis(1, at = pretty(range(t)), labels = rev(pretty(range(-t))), cex.axis=0.75)
lines(PDR~t,col="black",lty=2)
legend(
  "topright",
...
#Ymin <- 0.9*min(spe(t), PSR,ext(t)) - 0.03
#Ymax <- 1.1*max(spe(t), PSR,ext(t)) + 0.06
plot(spe(-t)~t,type="l",col=2,ylim=c(Ymin,Ymax),ylab = "", cex.lab = 0.75, cex.axis=0.75, xlab = bquote("Age (" * tau * ")"),
  xaxt = "n"
)
axis(1, at = pretty(range(t)), labels = rev(pretty(range(-t))),cex.axis=0.75)
lines(PSR~t,col=2,lty=2)
lines(ext(-t)~t,type="l",col=1)
lines(PER~t,col=1,lty=2)
legend(
  "topright",
  legend = c("speciation rate",
...
plot(PDR~t,type="l",lty=2,col="black",ylim=c(Ymin,Ymax),ylab = "", cex.lab = 0.75, cex.axis=0.75, xlab = bquote("Age (" * tau * ")"),
  xaxt = "n"
)
axis(1, at = pretty(range(t)), labels = rev(pretty(range(-t))),cex.axis=0.75)
lines(PSR~t,col=2,lty=2)
...
Here we introduce a slight difference in the amplitude, the degree to which rates vary, by increasing amplitude for speciation rate.
...
plot(spe[[i]](t)~t,type="l",col=2,ylim=c(Ymin,Ymax),ylab = "", cex.lab = 0.75, cex.axis=0.75, xlab = bquote("Age (" * tau * ")"),
  xaxt = "n"
)
axis(1, at = pretty(range(t)), labels = rev(pretty(range(-t))), cex.axis=0.75)
lines(ext[[i]](t)~t,col=1)
...
Here we show the effect of a slight difference in the period, the rate at which rates vary, by making speciation rate vary faster than extinction rate.
...
Finally we introduce a slight delay in the phase, the timing of the rate variation, by shifting the curve for speciation rate to the right.
