...
plot(spe[[i]](t)~rev(-t),type="l",col=2,ylim=c(Ymin,Ymax),ylab = "", cex.lab = 0.75, cex.axis=0.75, xlab = bquote("Age (" * tau * ")"),
  xaxt = "n"
)
axis(1, at = pretty(range(t)), labels = rev(pretty(range(-t))), cex.axis=0.75)
lines(ext[[i]](t)~rev(-t),col=1)
lines(div1(t)~rev(-t),col="black",lwd=2)
lines(PDR~rev(-t),col="black",lty=2)
if(i == 1){
  legend(
  "topright",
  legend = c("speciation rate",
